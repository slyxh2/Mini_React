...
function dispatchEvent(container: Container, eventType: string, e: Event) {
    ...
    const { bubble, capture } = collectPaths(<DOMElement>targetElement, container, eventType);
...
function triggerEventFlow(callbacks: EventCallback[], syntheticEvent: SyntheticEvent) {
...
function gerEventCallbackNameFromEventType(eventType: string): string[] | undefined {
    return {
        'click': ['onClickCapture', 'onClick']
    }[eventType]
}

function collectPaths(targetElement: DOMElement, container: Container, eventType: string): Paths {
    const paths: Paths = {
        capture: [],
        bubble: []
    };
